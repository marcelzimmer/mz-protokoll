...
//! MZ-Protokoll - Meeting-Protokoll-Editor
//!
...
/// Öffnet eine URL im Standard-Webbrowser (Windows, macOS und Linux).
...
/// Zentraler Anwendungszustand von MZ-Protokoll.
...
impl ProtokollApp {
    ...
    fn new(ctx: &egui::Context) -> Self {
        ...
        let has_omarchy = omarchy_farben_laden().is_some();
        ...
        Self {
            ...
            theme: if has_omarchy { Theme::Omarchy } else { Theme::Dunkel },
            save_path: None,
    ...
    /// Format: `MZ-Protokoll_<Titel>__<JJJJ-MM-TT>.md`
    fn dateinamen_erstellen(&self) -> String {
        ...
        format!("MZ-Protokoll_{}__{}.md", name_part, datum)
    }

    /// Serialisiert den aktuellen Protokollzustand als Markdown-String.
    /// Das Format ist spezifisch für MZ-Protokoll und wird von `markdown_parsen` wieder eingelesen.
    fn markdown_erstellen(&self) -> String {
        ...
        md.push_str("*Erstellt mit MZ-Protokoll von Marcel Zimmer — [www.marcelzimmer.de](https://www.marcelzimmer.de) | [X @marcelzimmer](https://x.com/marcelzimmer) | [GitHub @marcelzimmer](https://github.com/marcelzimmer)*\n");
    ...
    /// Liest einen MZ-Protokoll-Markdown-String ein und befüllt alle Felder
    ...
    fn markdown_parsen(&mut self, content: &str) {
        ...
        // Beide zunächst false, damit der Status-Parser eine exklusive Setzung
        // durchführen kann. Nach dem Parse-Durchlauf wird bei Bedarf "Entwurf" als
        // Default gesetzt, damit die UI-Invariante "genau eines aktiv" gilt.
            ...
            match section {
                ...
                Section::Teilnehmer => {
                    if let Some(rest) = trimmed.strip_prefix("- ") {
                        let (name, kuerzel) = name_kuerzel_parsen(rest);
                ...
                Section::Status => {
                    // Exklusiv: die jeweils andere Option deaktivieren, falls die Datei
                    // widersprüchlich beide [x] angekreuzt enthält.
        ...
        // UI-Invariante: genau eine Status-Checkbox aktiv. Wenn die Datei keinen
        // Status enthält oder beide leer sind, "Entwurf" als Default wählen.
        if !self.ist_entwurf && !self.ist_freigegeben {
            self.ist_entwurf = true;
        }
    ...
    /// Format: `MZ-Protokoll_<Titel>__<JJJJ-MM-TT>.pdf`
    fn pdf_dateinamen_erstellen(&self) -> String {
        let name_part: String = self.titel.chars().filter(|c| c.is_alphabetic()).collect();
        let datum = Local::now().format("%Y-%m-%d").to_string();
        format!("MZ-Protokoll_{}__{}.pdf", name_part, datum)
    }
    ...
    fn schrift_laden(&self) -> Option<genpdf::fonts::FontFamily<genpdf::fonts::FontData>> {
        ...
        // 2. Einzelne .ttf-Dateien (plattformspezifische Systemschriften)
        #[cfg(windows)]
    ...
    fn pdf_inhalt_hinzufuegen(&self, doc: &mut genpdf::Document) {
        ...
        if !entries.is_empty() {
            ...
            for e in &entries {
                ...
                if is_todo {
                    // Hintergrundhöhe schätzen — Zeichen-Umbruch in der Notiz-Spalte
                    // berücksichtigen, sonst deckt Grau nur den oberen Teil ab.
                    // Notiz-Spalte: 13/29 des 180mm-Inhaltsbereichs ≈ 80mm, minus Padding.
                    let notiz_spalte_mm = 180.0 * 13.0 / 29.0 - 4.0;
                    let wrapped_lines = gewrappte_zeilen_schaetzen(&e.notiz, notiz_spalte_mm);
                    // ~5mm Zeilenhöhe bei 9pt + 10mm Puffer für Padding und Schätz-Toleranz.
                    // Überschuss wird vom weißen Hintergrund der nächsten Nicht-TODO-Zeile abgedeckt.
                    let row_h = wrapped_lines as f64 * 5.0 + 10.0;
    ...
    fn pdf_generieren(&self, path: &std::path::Path, schriftfamilie: genpdf::fonts::FontFamily<genpdf::fonts::FontData>) {
        ...
        let gesamtseiten = {
            ...
            // Dieselben Margins wie im echten Render-Durchlauf (FusszeileDekorator),
            // damit die Seiten-Wrap-Berechnung identisch ausfällt und die Gesamtseitenzahl
            // in der Fußzeile korrekt ist.
            dekorator.set_margins(genpdf::Margins::trbl(20, 15, 20, 15));
        ...
        let druckdatum = Local::now().format("Druckdatum: %d.%m.%Y %H:%M:%S Uhr").to_string();
        let mut dok = genpdf::Document::new(schriftfamilie);
        let pdf_titel = if self.titel.is_empty() {
            "MZ-Protokoll".to_string()
        } else {
            format!("{} — MZ-Protokoll von Marcel Zimmer (www.marcelzimmer.de)", self.titel)
        };
        dok.set_title(&pdf_titel);
        dok.set_page_decorator(FusszeileDekorator::new(gesamtseiten, druckdatum));
...
/// Schätzt die Anzahl der durch Wortumbruch in der gegebenen Spaltenbreite
/// tatsächlich belegten Zeilen. Wird für die Höhe des grauen TODO-Hintergrunds
/// im PDF gebraucht, damit auch lange einzeilige Notizen vollständig hinterlegt
/// werden und nicht nur ihr oberer Rand.
fn gewrappte_zeilen_schaetzen(text: &str, spalte_mm: f64) -> usize {
    // ~1.7 mm mittlere Zeichenbreite bei 9pt Bold (Liberation Sans u. Ä.).
    let zeichen_pro_zeile = ((spalte_mm / 1.7) as usize).max(1);
    text.split('\n')
        .map(|line| line.chars().count().div_ceil(zeichen_pro_zeile).max(1))
        .sum::<usize>()
        .max(1)
}
...
/// mit App-Name (links), Druckdatum (Mitte) und Seitenzahl (rechts) hinzu.
struct FusszeileDekorator {
    ...
    gesamtseiten: usize,
    /// Druckdatum als formatierter String.
    druckdatum: String,
}

impl FusszeileDekorator {
    /// Erstellt einen neuen Fußzeile-Dekorierer mit der bekannten Gesamtseitenzahl
    /// und dem Druckzeitstempel.
    fn new(gesamtseiten: usize, druckdatum: String) -> Self {
        Self {
...
impl genpdf::PageDecorator for FusszeileDekorator {
    fn decorate_page<'a>(
        ...
        let fuss_text_y = rohseite_hoehe - 15.0;
        let linker_rand = 15.0;
        let rechter_rand = 15.0;
        let fuss_stil = genpdf::style::Style::new().with_font_size(7);
        let fuss_stil_fett = genpdf::style::Style::new().bold().with_font_size(7);
        let grau = genpdf::style::Color::Greyscale(150);

        // Trennlinie (echte Vektorlinie von linkem bis rechtem Rand)
        let linie_y = rohseite_hoehe - 17.0;
        area.draw_line(
            vec![
                genpdf::Position::new(linker_rand, linie_y),
                genpdf::Position::new(rohseite_breite - rechter_rand, linie_y),
            ],
            genpdf::style::Style::new().with_color(grau),
        );

        // Links: "MZ-Protokoll" (fett)
        let _ = area.print_str(
            &context.font_cache,
            genpdf::Position::new(linker_rand, fuss_text_y),
            fuss_stil_fett,
            "MZ-Protokoll",
        );

        // Mitte: Druckdatum
        // "Druckdatum: DD.MM.YYYY HH:MM:SS Uhr" ≈ 37 Zeichen à ~1.5mm bei 7pt → ~55mm
        let datum_breite = self.druckdatum.len() as f64 * 1.5;
        let datum_x = rohseite_breite / 2.0 - datum_breite / 2.0;
        let _ = area.print_str(
            &context.font_cache,
            genpdf::Position::new(datum_x, fuss_text_y),
            fuss_stil,
            &self.druckdatum,
        );

        // Rechts: "Seite X von Y"
        let seite_text = format!("Seite {} von {}", self.aktuelle_seite, self.gesamtseiten);
        let seite_breite = seite_text.len() as f64 * 0.44 * 7.0 * 0.353;
        let _ = area.print_str(
            &context.font_cache,
            genpdf::Position::new(rohseite_breite - rechter_rand - seite_breite, fuss_text_y),
            fuss_stil,
            &seite_text,
        );
...
impl eframe::App for ProtokollApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().frame(panel_frame).show(ctx, |ui| {
            // Toolbar oben rechts: Hamburger-Menü
                    ...
                    for &(label, shortcut, is_sep) in menu_items {
                        ...
                        if clicked {
                            match label {
                                ...
                                "Beenden" => self.show_quit_dialog = true,
                                _ => {}
                            }
                            ui.close_menu();
                        }
                ...
                ui.horizontal(|ui| {
                    ...
                    for (idx, s) in sicherheiten.iter().enumerate() {
                        ...
                        if idx < last_idx {
                            ...
                            if clicked {
                                self.sicherheit = if checked { *s } else { Sicherheit::Intern };
                            }
                        } else if ui.checkbox(&mut checked, label).clicked() {
                            self.sicherheit = if checked { *s } else { Sicherheit::Intern };
                        }
                    }
                });
                        ...
                        for i in 0..entry_len {
                            ...
                            ui.with_layout(egui::Layout::top_down(egui::Align::LEFT), |ui| {
                                ...
                                egui::ComboBox::from_id_salt(format!("art_{i}"))
                                    ...
                                    .width(art_w)
                                    .show_ui(ui, |ui| {
                                        let prev_art = self.eintraege[i].art;
                                        for art in Art::all() {
                                            ...
                                            .show_ui(ui, |ui| {
                                                ...
                                                for k in &alle_kuerzel {
                                                    if ui
                                                        ...
                                                        .clicked()
                                                    {
                                                        self.eintraege[i].kuemmerer.clone_from(k);
                                                    }
                                                }
                                            });
                                    } else {
                            ...
                            ui.with_layout(egui::Layout::top_down(egui::Align::LEFT), |ui| {
                                ui.horizontal(|ui| {
                                    ...
                                    ui.add_space(10.0);
                                    if entry_len > 1
                                        && ui.add_sized([20.0, 20.0], egui::Button::new(
                                            RichText::new("×").color(egui::Color32::from_rgb(231, 76, 60)),
                                        )).clicked()
                                    {
                                        entry_remove = Some(i);
                ...
                // Cursor hoch/runter zwischen Notiz-Feldern
                {
                    ...
                    if let Some((prev_i, prev_cursor)) = prev_notiz_focus {
                        if prev_i < self.eintraege.len() {
                            ...
                            // CCursor::index ist ein Zeichen-Index (nicht Byte-Index);
                            // für das Slicing in Byte-Index umrechnen.
                            let byte_idx = text
                                .char_indices()
                                .nth(prev_cursor)
                                .map_or(text.len(), |(i, _)| i);
                            let on_first = !text[..byte_idx].contains('\n');
                            let on_last = !text[byte_idx..].contains('\n');
        ...
        if self.show_about_dialog {
            let mut open = true;
            egui::Window::new("Über MZ-Protokoll")
                .open(&mut open)
                .collapsible(false)
                .resizable(false)
                ...
                .show(ctx, |ui| {
                    ...
                    ui.vertical_centered(|ui| {
                        ui.add_space(12.0);
                        ui.label(RichText::new("MZ-Protokoll").strong().size(24.0));
                        ui.add_space(4.0);
            ...
            if !open { self.show_about_dialog = false; }
        }
        ...
        if self.show_quit_dialog {
            let taste_ja = ctx.input(|i| {
                i.key_pressed(egui::Key::Enter) || i.key_pressed(egui::Key::J)
            });
            let taste_nein = ctx.input(|i| {
            ...
            egui::Window::new("confirm_exit_dlg")
                ...
                .frame(
                    egui::Frame::new()
                        ...
                        .stroke(egui::Stroke::new(2.0, ctx.style().visuals.selection.bg_fill))
                        .corner_radius(egui::CornerRadius::same(12))
                        .inner_margin(egui::Margin::same(24)),
                )
                .show(ctx, |ui| {
                    ui.label(RichText::new("MZ-Protokoll wirklich beenden?").size(20.0).strong());
                    ui.add_space(16.0);
                    ui.horizontal(|ui| {
                        if ui.add(egui::Button::new(
                            RichText::new("  Ja  ").size(18.0)
                        ).corner_radius(egui::CornerRadius::same(8))).clicked() || taste_ja {
                            ctx.send_viewport_cmd(egui::ViewportCommand::Close);
                        }
                        ui.add_space(12.0);
                        if ui.add(egui::Button::new(
                            RichText::new("  Nein  ").size(18.0)
                        ).corner_radius(egui::CornerRadius::same(8))).clicked() || taste_nein {
                            self.show_quit_dialog = false;
                        }
                    });
                });
